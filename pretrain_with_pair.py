...
from trainer.fintuner_withpair import AAGBARTFINETUNER_WITH_PAIR
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--train_dataset", type=str, default="./data/train_pair_diet.csv", help="train dataset for train bert")
    parser.add_argument("-t", "--test_dataset", type=str, default="./data/val_pair_diet.csv", help="test set for evaluate train set")

    parser.add_argument("-o", "--output_path",  type=str, default="./result/no_ult+no_augment/lr=5e5/", help="ex)output/report.txt")
    ...
    parser.add_argument("-at", "--attribute_type", default="allergy", type=str, help="types: ['allergy', 'ingredient']")
    ...
    parser.add_argument("-agm","--agm", type=bool, default=True, help="allow to do data augmentation")
    parser.add_argument("-agx","--agm_x", type=int, default=50, help="setting the number of multiply for the number of augmented data")
    ...
    output_path = args.output_path #+ "change_only/agm_{}/auto_loss/udm_{}/uls_{}/".format(args.agm, args.u_decode, args.u_lambda)
    ...
    #target_num_corpus = int(init_num_corpus * args.agm)
    #final_path = output_path + "final_augmented_data.csv"
    
    for epoch in tqdm(range(args.epochs)):

        #train_afc_ratio, augmented_data, original_data, original_allergy = trainer.train(epoch)
        
        train_correct_ratio = trainer.train(epoch)
        ...
        #finetuner.update(epoch)
        #finetuner.save(epoch, output_path)

        if test_data_loader is not None:
            test_afc_ratio = trainer.test(epoch)
            if test_afc_ratio > best_correct:
                trainer.save_best(epoch, output_path)
                best_correct = test_afc_ratio
